...
interface ItemData {
    ...
    pickupLocations: unknown; // JSON type from schema returns unknown
    images: string[] | null;
...
function parsePickupLocations(data: unknown): PickupLocation[] {
    if (!data) return [];
...
export function ItemEditForm({ item, categories }: { item: ItemData; categories: Category[] }) {
    ...
    // Pickup locations state - safely parsed
    const [pickupLocations, setPickupLocations] = useState<PickupLocation[]>(
        parsePickupLocations(item.pickupLocations)
    );
    ...
    return (
        <form action={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div>
                            ...
                            <div className="relative rounded-md shadow-sm">
                                <input
                                    ...
                                    min="0"
                                    max="100"
                                    defaultValue={item.discountRate3Days || ''}
                        ...
                        <div>
                            ...
                            <div className="relative rounded-md shadow-sm">
                                <input
                                    ...
                                    min="0"
                                    max="100"
                                    defaultValue={item.discountRate7Days || ''}
                                    className="block w-full rounded-lg pr-8 sm:text-sm px-4 py-2 border border-gray-300 focus:border-green-500 focus:ring-green-500"
                                    placeholder="0"
                                />
                                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                    ...
                    <div className="flex gap-4">
                        <div className="flex-1">
                            ...
                            <input
                                type="date"
                                name="availableFrom"
                                defaultValue={formatDateForInput(item.availableFrom)}
                        ...
                        <div className="flex-1">
                            ...
                            <input
                                type="date"
                                name="availableTo"
                                defaultValue={formatDateForInput(item.availableTo)}
                                className="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm px-4 py-2 border"
                            />
                        </div>
                    </div>
            ...
            <div className="bg-gray-50 px-8 py-4 flex items-center justify-end gap-3 border-t border-gray-100">
                <Link href="/member" className="px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900">
                    取消
                </Link>
                <button
                    type="submit"
